...
const Dashboard = () => {
  ...
  const doneCount = assets?.filter((a) => a.status === "DONE" || a.status === "VIDEO_RENDERED").length ?? 0;

  // Determine link based on status
  const getAssetLink = (asset: any) => {
    if (["VARIANTS_READY", "IMAGE_READY"].includes(asset.status)) return `/assets/${asset.id}/variants`;
    if (["RENDERING", "DONE"].includes(asset.status)) return `/assets/${asset.id}/render`;
    return `/assets/${asset.id}/variants`;
  };
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link to="/assets/new">
          <Button className="gap-2"><Plus className="w-4 h-4" /> Nuevo Video</Button>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {[
          ...
          { label: "Videos Completos", value: String(doneCount) },
        ].map((stat, i) => (
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 border border-border rounded-xl bg-card">
          ...
          <Link to="/assets/new"><Button className="mt-4 gap-2"><Plus className="w-4 h-4" /> Nuevo Video</Button></Link>
        </div>
      ) : (
        <div className="rounded-xl border border-border overflow-hidden">
          <div className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-5 py-3 bg-muted/30 text-xs text-muted-foreground font-medium uppercase tracking-wider">
            <span>Video URL</span><span>Estado</span><span>Fecha</span><span></span>
          </div>
          {assets.map((asset, i) => (
            <motion.div key={asset.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.05 + i * 0.03 }}>
              <Link to={getAssetLink(asset)}
